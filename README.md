...
**AI use:** OpenAI Codex was used to help write and revise the Python code, explain the methods and equations, and organize and edit this report. It also helped run the initial camera calibration. AI assistance was not limited to formatting. The recorded calibration results and the expected behavior of the other methods are identified separately below.

Setup

This assignment covers camera calibration, noise filtering, thresholding and morphology. The code is in [hw1.ipynb](homework/hw1.ipynb) and [homework/utils](homework/utils/).

1. Put the repository in a local folder. Keep `homework/hw1.ipynb`, `homework/utils/`, and `images/` in their current relative locations. Use the supplied images without resizing or editing them.
2. From the repository root in PowerShell, use the commands below. Create the virtual environment only if `.venv` does not already exist.
3. Open the notebook and select the `.venv` kernel. Questions 1-4 can each run independently. Run question 4 before question 5 because its returned `segmentation` dictionary supplies the masks. To reproduce everything, run the five question cells in numerical order and save the notebook with outputs.
4. Questions 2-5 save 35 comparison PNGs in `homework/filter_results`: 4 Gaussian/median comparisons, 4 median/rank comparisons, 3 threshold comparisons, and 24 morphology comparisons. Each figure includes its input and all labeled outputs. Calibration prints numerical results instead of saving figures.
5. Record the package versions from your actual run using the final command below. Dependencies are not pinned, so a new installation is not a guarantee of identical numerical results. Attach the resulting environment file to the submission.
...
After the run, in a terminal with the same environment active:
...
Implementation: [CalibrateCamera.py](homework/utils/CalibrateCamera.py) and [CalibrationComparison.py](homework/utils/CalibrationComparison.py). Only folder 1 is used. The three groups are subsets of a single recording, so this does not demonstrate three independently acquired data sets if the instructor requires separate recordings.
...
![Camera man comparison](homework/filter_results/camera_man_w_noise_comparison.png)

![Salt and pepper comparison](homework/filter_results/SaltAndPepperNoise_comparison.png)

![Gaussian noise comparison](homework/filter_results/GaussianNoise_comparison.png)

![Uniform noise comparison](homework/filter_results/UniformNoise_comparison.png)
...
![Camera man rank comparison](homework/filter_results/camera_man_w_noise_rank.png)

![Salt and pepper rank comparison](homework/filter_results/SaltAndPepperNoise_rank.png)

![Gaussian rank comparison](homework/filter_results/GaussianNoise_rank.png)

![Uniform rank comparison](homework/filter_results/UniformNoise_rank.png)
...
![Coins1 threshold comparison](homework/filter_results/coins1_threshold.png)

![Coins2 threshold comparison](homework/filter_results/coins2_threshold.png)

![Screws threshold comparison](homework/filter_results/screws_threshold.png)
...
The notebook displays and saves 24 labeled comparison figures: three images x two threshold methods x two shapes x two sizes. Each figure contains the original, threshold mask, erosion, dilation, opening, closing, open-then-close and close-then-open outputs. Filenames follow `image_method_shape_size_morphology.png`. All panels are included in the saved figures in [filter_results](homework/filter_results/).

| Image / threshold | Rectangle 3x3 | Rectangle 5x5 | Ellipse 3x3 | Ellipse 5x5 |
| --- | --- | --- | --- | --- |
| coins1 / global | ![coins1 global rectangle 3](homework/filter_results/coins1_global_rectangle_3_morphology.png) | ![coins1 global rectangle 5](homework/filter_results/coins1_global_rectangle_5_morphology.png) | ![coins1 global ellipse 3](homework/filter_results/coins1_global_ellipse_3_morphology.png) | ![coins1 global ellipse 5](homework/filter_results/coins1_global_ellipse_5_morphology.png) |
| coins1 / adaptive | ![coins1 adaptive rectangle 3](homework/filter_results/coins1_adaptive_rectangle_3_morphology.png) | ![coins1 adaptive rectangle 5](homework/filter_results/coins1_adaptive_rectangle_5_morphology.png) | ![coins1 adaptive ellipse 3](homework/filter_results/coins1_adaptive_ellipse_3_morphology.png) | ![coins1 adaptive ellipse 5](homework/filter_results/coins1_adaptive_ellipse_5_morphology.png) |
| coins2 / global | ![coins2 global rectangle 3](homework/filter_results/coins2_global_rectangle_3_morphology.png) | ![coins2 global rectangle 5](homework/filter_results/coins2_global_rectangle_5_morphology.png) | ![coins2 global ellipse 3](homework/filter_results/coins2_global_ellipse_3_morphology.png) | ![coins2 global ellipse 5](homework/filter_results/coins2_global_ellipse_5_morphology.png) |
| coins2 / adaptive | ![coins2 adaptive rectangle 3](homework/filter_results/coins2_adaptive_rectangle_3_morphology.png) | ![coins2 adaptive rectangle 5](homework/filter_results/coins2_adaptive_rectangle_5_morphology.png) | ![coins2 adaptive ellipse 3](homework/filter_results/coins2_adaptive_ellipse_3_morphology.png) | ![coins2 adaptive ellipse 5](homework/filter_results/coins2_adaptive_ellipse_5_morphology.png) |
| screws / global | ![screws global rectangle 3](homework/filter_results/screws_global_rectangle_3_morphology.png) | ![screws global rectangle 5](homework/filter_results/screws_global_rectangle_5_morphology.png) | ![screws global ellipse 3](homework/filter_results/screws_global_ellipse_3_morphology.png) | ![screws global ellipse 5](homework/filter_results/screws_global_ellipse_5_morphology.png) |
| screws / adaptive | ![screws adaptive rectangle 3](homework/filter_results/screws_adaptive_rectangle_3_morphology.png) | ![screws adaptive rectangle 5](homework/filter_results/screws_adaptive_rectangle_5_morphology.png) | ![screws adaptive ellipse 3](homework/filter_results/screws_adaptive_ellipse_3_morphology.png) | ![screws adaptive ellipse 5](homework/filter_results/screws_adaptive_ellipse_5_morphology.png) |
